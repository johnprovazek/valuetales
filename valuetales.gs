...
let startDate = new Date("01/01/2000"); // Start date in the format 'MM/DD/YYYY'.
...
//
// coverPhotoWeekOverride: Handles how to select the cover photo for the Google Doc. Setting this value to a value
// between 1-50 will override the random image for the cover photo and take the image from the corresponding week.
// Setting this value to -1 will keep the random image for the cover photo.
//
// googleDocLinkOverride: Handles how to send the Google Doc at the end of the year. Setting this value to "auto" will
// have the Google Doc auto-generated and sent without a review. Setting this value to "skip" will skip sending the book
// on week 52. Setting this value to a Google Doc link will have your Google Doc link sent instead. This is helpful if
// you want to manually make any edits before sending the Google Doc to your recipient.
let recipients = [
  {
    firstName: "", // First name of the recipient.
    lastName: "", // Last name of the recipient.
    email: "", // Email address of the recipient.
    coverPhotoWeekOverride: -1, // Handles how to select the cover photo for the Google Doc.
    googleDocLinkOverride: "auto", // Handles how to send the Google Doc at the end of the year.
  },
];
...
// Google Docs values.
// This project is currently only setup to build a 5.5 inch x 8.5 inch book with 0.5 inch margins.
// Changing some of the values below will result in unknown behavior.
let pointsInInch = 72; // Google Docs default value.
let pixelsInInch = 96; // Google Docs default value.
let pageHeightInchFull = 8.5; // Page height in inches.
let pageWidthInchFull = 5.5; // Page width in inches.
let pageMarginInch = 0.5; // Page margins around the whole document in inches.
let pageHeightInch = pageHeightInchFull - 2 * pageMarginInch; // Page height of the editable area in inches.
let pageHeightUnit = 1800; // This 'unit' is specific to this project. This represents the height of the editable area.
let pageWidthUnit = 1080; // This 'unit' is specific to this project. This represents the width of the editable area.

// Conversions between all Google Docs units used in this project.
const conversions = {
...
function sendEmail(data) {
  ...
  GmailApp.sendEmail(data.email, data.subject, body, {
    htmlBody: body,
...
function askQuestions() {
  ...
  if (startDate.getFullYear() <= 2023) {
    // Checking to see if the user (you) updated the startDate variable.
  ...
  if (weekNumber === 0) {
    ...
    // Special last week completion message.
    for (let i = 0; i < recipients.length; i++) {
      ...
      // Sending message to the sender (you) to remind them that they have one week until the finished book is sent.
      sendEmail({
    ...
    for (let i = 0; i < recipients.length; i++) {
      ...
      if (override === "skip") {
        ...
        // Send message to the sender (you) that they selected an invalid value for googleDocLinkOverride.
        // The book was not sent to the recipient.
        sendEmail({
          subject:
            "Invalid valuetales googleDocLinkOverride value for " +
            recipients[i].firstName +
            " " +
            recipients[i].lastName,
          email: senderEmail,
          template: {
            paragraph: "invalid",
            recipientFirstName: recipients[i].firstName,
    ...
    // Special message to sender letting them know to disable this service.
    sendEmail({
...
// Function to get the estimated paragraph height for a given paragraph and font size based on the total page height of
// 1800 units. The total page height of 1800 units was derived by using the monospaced font Courier in a 5.5 inch x 8.5
// inch Google Doc with 0.5 inch margins. The font sizes 10pt, 15pt, 18pt, and 30pt fit the document an exact number of
// times down and across. This is why the font Courier was chosen. 1800 units is chosen as the page height due to 450
// being the lowest common multiple (LCM) of 45, 30, 25, and 15 which are the total number of lines in the document a
// 10pt, 15pt, 18pt, and 30pt font can fit in respectively. The LCM of 450 was then multiplied by 4 to get 1800 units
// for the height to make the page height divisible by more values for more flexibility. There is no method in the Apps
// Script service for Google Docs to get a Paragraph's height making this function necessary to be able to keep track of
// the vertical alignment of items in the Document. The best workaround for this problem is to determine the number of
// lines in a Paragraph by approximating the Google Docs line wrap algorithm. After finding the number of lines in a
// Paragraph this value can then be multiplied by the line height for the specific font size of the Paragraph. The
// downside to this workaround is that there may be inconsistencies in this approximated line wrap algorithm and the
// Google Docs line wrap algorithm resulting in incorrect Paragraph line counts.
function getParagraphHeight(paragraph, fontSize, indented) {
  const lineWidths = {
    10: 54, // 10pt Courier font fits 54 characters per line in a 5.5 inch x 8.5 inch Google Doc with 0.5 inch margins.
    15: 36, // 15pt Courier font fits 36 characters per line in a 5.5 inch x 8.5 inch Google Doc with 0.5 inch margins.
    18: 30, // 18pt Courier font fits 30 characters per line in a 5.5 inch x 8.5 inch Google Doc with 0.5 inch margins.
    30: 18, // 30pt Courier font fits 18 characters per line in a 5.5 inch x 8.5 inch Google Doc with 0.5 inch margins.
  };
  const lineHeights = {
    10: 40, // 10pt Courier font fits 45 lines in a 5.5 inch x 8.5 inch Google Doc with 0.5 inch margins. 1800/45 = 40
    15: 60, // 15pt Courier font fits 30 lines in a 5.5 inch x 8.5 inch Google Doc with 0.5 inch margins. 1800/30 = 60
    18: 72, // 18pt Courier font fits 25 lines in a 5.5 inch x 8.5 inch Google Doc with 0.5 inch margins. 1800/25 = 72
    30: 120, // 30pt Courier font fits 15 lines in a 5.5 inch x 8.5 inch Google Doc with 0.5 inch margins. 1800/15 = 120
  };
  const indentCharacterCount = {
    10: 3, // 10pt Courier font fits 3 characters in a 5.5 inch x 8.5 inch Google Doc with 0.5 inch margins with a 0.25 inch tab.
    15: 2, // 15pt Courier font fits 2 characters in a 5.5 inch x 8.5 inch Google Doc with 0.5 inch margins with a 0.25 inch tab.
    18: 2, // 18pt Courier font fits 2 characters in a 5.5 inch x 8.5 inch Google Doc with 0.5 inch margins with a 0.25 inch tab. (This is the only font size where the tab is not an exact character count)
    30: 1, // 30pt Courier font fits 1 characters in a 5.5 inch x 8.5 inch Google Doc with 0.5 inch margins with a 0.25 inch tab.
  };
  ...
  if (indented) {
    if (words.length && words[0].length > indentLineWidth) {
      // Special case where word is longer than indented line.
...
function buildBook(recipient) {
  ...
  threads.sort(function (a, b) {
    // Sorts email threads by the question number.
    if (a.getFirstMessageSubject() < b.getFirstMessageSubject()) {
      return -1;
    }
    if (a.getFirstMessageSubject() > b.getFirstMessageSubject()) {
      return 1;
  ...
  for (let i = 0; i < threads.length; i++) {
    ...
    if (messages.length >= 2) {
      for (let j = messages.length - 1; j >= 1 && (!imageFound || !responseFound); j--) {
        // Iterating through the thread's messages until a suitable image and response are found.
        // Gathering and formatting the email body into paragraphs.
        if (!responseFound) {
          let messageBody = messages[j].getPlainBody();
          let regexList = [
            // List of regexes used to clean email body by replacing them with an empty string.
            // This is where you will need to remove items such as email signatures or previous messages in the thread.
          ...
          // Split paragraphs up by occurrences of consecutive '\r' and '\n' characters.
          let paragraphs = messageBody.split(/(?:\r\n){2,}/gm);
          let paragraphsFiltered = paragraphs.filter(function (a) {
            return a !== "";
          }); // Removing any empty paragraphs.
    ...
    if (tale.responseParagraphs.length) {
      // If there is a suitable paragraph add the 'tale'.
      tales.push(tale);
    }
  }
  ...
  let date = new Date();
  let doc = DocumentApp.create(
    "valuetales-" + recipient.firstName + "-" + recipient.lastName + "-" + date.toUTCString()
  );
  ...
  // Handles adding an image and scaling it to fit in the given height on the page. Much of the logic in this function
  // is a workaround for limitations in the Apps Script service for Google Docs. A PositionedImage is required to be
  // anchored to a Paragraph, but this results in an unwanted line after the image that can not be removed. The best
  // workaround for this is to add an empty line above the image in addition to this line to make the spacing look more
  // intentional and even. There is also no way to set the 'margins from text' on an image through the Apps Script
  // service for Google Docs. The default 'margins from text' on an image is 0.02 inches so this value will need to be
  // subtracted from the height of the image to keep the units consistent. The 'margins from text' on an image doesn't
  // apply to the paragraph before the image so the paragraph before the image will need to add spacing after the
  // paragraph to compensate. The 'unit' 60, which is the line height of the font size 15pt, is used because when this
  // 'unit' value is converted to inches the converted value will be in increments of 0.25 inches. Google Docs limits
  // the width and height values in inches of images to two decimal places. Increments of 0.25 inches will keep the size
  // of images accurate.
  let addImage = (image, height) => {
    let heightMarginsAndLine = 60 - 2 * convertUnit(0.02, "inch", "unit");
    // Removing the height of the lines above and below and the margins.
    let trimHeight = height - 60 * 2 - 2 * convertUnit(0.02, "inch", "unit");
    if (trimHeight < heightMarginsAndLine) {
      // Minimum height check.
      console.log("Invalid image height. Skipping adding image. May result in errors.");
      return 0;
    } else {
      // This paragraph is for the space above an image.
      let emptyParagraph = buildParagraph("", 15, "normal", "center");
      // This is to counteract the margins from text on the image.
      emptyParagraph.object.setSpacingAfter(convertUnit(0.02, "inch", "point"));
      // This paragraph is necessary to contain the image but also places an unwanted line below the image.
      let imageParagraph = buildParagraph("", 15, "normal", "center");
  ...
  let coverTaleOverride = tales.find(
    (tale) => tale.weekNumber === recipient.coverPhotoWeekOverride && tale.image !== null
  );
  if (recipient.coverPhotoWeekOverride >= 1 && recipient.coverPhotoWeekOverride <= 50 && coverTaleOverride) {
    ...
    let talesWithImages = tales.filter(function (a) {
      return a.image !== null;
    });
  ...
  if (
    getParagraphHeight(titlePageDescriptionText, 10, true) + titlePageImage.height + titlePageNameParagraph.height >
    pageHeightUnit
  ) {
  ...
  for (let i = 0; i < tales.length; i++) {
    ...
    for (let j = 0; j < tales[i].responseParagraphs.length; j++) {
      ...
      if (j !== tales[i].responseParagraphs.length - 1) {
        // Adding a space between paragraphs.
        buildParagraph("", 10, "normal", "left");
      }
    }
    if (i !== tales.length - 1 && responseParagraph.object) {
      // Adding a page break between page questions.
